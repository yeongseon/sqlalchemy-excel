...
import pytest
from sqlalchemy import Column, Integer, MetaData, String, Table, bindparam, create_engine, select
...
from sqlalchemy_excel.dml import OnConflictClause, OnConflictDoNothing, OnConflictDoUpdate, insert
...
def test_compiler_validate_join_tree_requires_on_clause() -> None:
    bad_join = SimpleNamespace(left=object(), right=object(), onclause=None, full=False, isouter=False)
...
def test_compiler_quote_aware_helpers_cover_escaped_quotes() -> None:
    ...
    assert depth == 0
    assert ExcelCompiler._has_top_level_compound_op("SELECT 'UNION''ALL' FROM t") is False
...
def test_compiler_strip_order_and_limit_helpers_with_quoted_and_embedded_keywords() -> None:
...
def test_compiler_on_conflict_do_nothing_without_target_text(tmp_xlsx: str, users_table: Table) -> None:
    engine = create_engine(f"excel:///{tmp_xlsx}")
    compiler_inst = cast(Any, select(users_table).compile(dialect=engine.dialect))
    clause = OnConflictDoNothing(index_elements=None)
    assert compiler_inst.visit_on_conflict_do_nothing(clause) == "ON CONFLICT DO NOTHING"
    engine.dispose()
...
def test_compiler_upsert_bindparam_null_type_and_extra_key_warns(
    ...
    engine = create_engine(f"excel:///{tmp_xlsx}")
    stmt = insert(users_table).values(id=1, name="a", age=1).on_conflict_do_update(
        index_elements=["id"],
...
def test_ddl_compiler_error_paths_and_alternative_name_sources(tmp_xlsx: str) -> None:
    ...
    with pytest.raises(CompileError, match="does not support schemas"):
        compiler._format_alter_table_name(SimpleNamespace(schema="s", table_name="users"))
    with pytest.raises(CompileError, match="requires a table name"):
        compiler._format_alter_table_name(SimpleNamespace(schema=None, table_name=None, table=None))
...
def test_dialect_sync_alter_table_add_float_maps_to_real(monkeypatch: pytest.MonkeyPatch) -> None:
    writes: list[list[dict[str, Any]]] = []

    def read_table_metadata(_conn: Any, _table: str) -> list[dict[str, Any]]:
        return [{"name": "id", "type_name": "INTEGER", "nullable": False, "primary_key": True}]

    def get_columns(_conn: Any, _table: str) -> list[dict[str, Any]]:
        return [{"name": "id", "type": "INTEGER"}, {"name": "amount", "type": "FLOAT"}]

    def write_table_metadata(_conn: Any, _table: str, columns: list[dict[str, Any]]) -> None:
    ...
    dialect = ExcelDialect()
    cursor = SimpleNamespace(connection=object())
    dialect._sync_alter_table_metadata(cursor, "ALTER TABLE users ADD COLUMN amount FLOAT")
...
def test_dialect_sync_alter_table_rename_unknown_column_keeps_maps_stable(
    ...
    assert writes[-1] == [{"name": "new_name", "type_name": "TEXT", "nullable": True, "primary_key": False}]
